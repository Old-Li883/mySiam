...
def main():
    ...
    if trainINFO['ISTRUE']:
        ...
        os.system('CUDA_VISIBLE_DEVICES={0} python ./siamese_tracking/train_siamfc.py --cfg {1} --gpus {2} --workers {3} 2>&1 | tee logs/siamrpn_train.log'
    ...
    if testINFO['ISTRUE']:
        ...
        os.system('mpiexec -n {0} python ./siamese_tracking/test_epochs.py --arch {1} --start_epoch {2} --end_epoch {3} --gpu_nums={4} --threads {0} --dataset {5}   2>&1 | tee logs/siamfc_epoch_test.log'
                  .format(testINFO['THREADS'], trainINFO['MODEL'], testINFO['START_EPOCH'], testINFO['END_EPOCH'],
                          (len(info['GPUS']) + 1) // 2, testINFO['DATA']))
