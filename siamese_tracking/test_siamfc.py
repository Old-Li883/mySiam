...
def track(tracker, net, video, args):
    ...
    if args.epoch_test: # 每个epoch建立一个文件
    ...
    # 用box记录跟踪结果，整型是一些非跟踪状态值的记录
    regions = []
    image_files, gt = video['image_files'], video['gt'] # 读取所有的图片名，和groundTruth
    for f, image_file in enumerate(image_files): # f是索引值，image_file是图片路径
        im = cv2.imread(image_file) # 读取图片
        if len(im.shape) == 2: # 如果是灰度图，就要转成彩色图
        ...
        if f == start_frame:  # init
            cx, cy, w, h = get_axis_aligned_bbox(gt[f]) # gt在原始图像的上的坐标，函数是将左上角的坐标转成中心坐标
            target_pos = np.array([cx, cy]) # 初始帧box中心坐标
            target_sz = np.array([w, h]) # 初始帧box宽高
            # 初始化跟踪器
            state = tracker.init(im, target_pos, target_sz, net)  # init tracker
            location = cxy_wh_2_rect(state['target_pos'], state['target_sz']) # 转换成左上角的坐标，并做防止越界处理

            # 第一帧将groudtruth直接加入
            regions.append(1 if 'VOT' in args.dataset else gt[f])
        elif f > start_frame:  # tracking，进入跟踪
            state = tracker.track(state, im)
            location = cxy_wh_2_rect(state['target_pos'], state['target_sz']) # 转换成左上角的坐标，并做防止越界处理
            # 计算预测值与gt的重叠度，如果没有则认为跟丢了
    ...
    with open(result_path, "w") as fin:
        ...
        if 'VOT' in args.dataset:
            ...
            for x in regions:
                p_bbox = x.copy()
                # 这里的x,y加1，与前面读取时减一相对应，写下的东西用逗号隔开
                fin.write(
