...
if __name__ == '__main__':
    ...
    for f in files:
        ...
        gt_path= os.path.join(gt_dir, 'sot2')
        ...
        for target_txt in t_path:
            if target_txt in pre_targets_txt:
                target_index = target_txt.split(".")[0]
                gt_full_path = os.path.join(gt_path,target_txt)
                gt = pd.read_table(gt_full_path,sep=',',header=None)
                gt.columns = ['g_x','g_y','g_w','g_h','frame']
                gt['frame'] = gt['frame'].apply(lambda x:str(x).zfill(6)+'.jpg')
                gt['g_x'] = gt['g_x'] + gt['g_w'] / 2
                gt['g_y'] = gt['g_y'] + gt['g_h'] / 2
                gt = gt.set_index(['frame'])

                pre_full_path = os.path.join(pre_path,target_txt)
                pre = pd.read_table(pre_full_path,sep=',',header=None)
                pre.columns = ['p_x','p_y','p_w','p_h','frame']
                pre = pre.set_index(['frame'])
