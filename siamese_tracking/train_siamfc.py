...
def parse_args():
    ...
    parser.add_argument('--cfg',
                        required=True,
                        type=str,
                        default='experiments/train/SiamFC.yaml',
...
def lr_decay(cfg, optimizer):
    if cfg.SIAMFC.TRAIN.LR_POLICY == 'exp':
        ...
        scheduler = np.logspace(math.log10(cfg.SIAMFC.TRAIN.LR),
                                math.log10(cfg.SIAMFC.TRAIN.LR_END),
                                cfg.SIAMFC.TRAIN.END_EPOCH)
    else:
...
def main():
    ...
    model = models.__dict__[
        config.SIAMFC.TRAIN.MODEL]()  # build model，将模型信息变成字典的形式
    model = load_pretrain(model, './pretrain/{}'.format(
        config.SIAMFC.TRAIN.PRETRAIN))  # load pretrain
