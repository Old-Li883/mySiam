...
from models.modelIso import BCEWeightLoss
...
# record loss
        ...
        if (iter + 1) % cfg.PRINT_FREQ == 0:
            logger.info(
                'Epoch: [{0}][{1}/{2}] lr: {lr:.7f}\t Batch Time: {batch_time.avg:.3f}s \t Data Time:{data_time.avg:.3f}s \t Loss:{loss.avg:.5f}'
                .format(epoch,
                        iter + 1,
                        len(train_loader),
                        lr=cur_lr,
            ...
            print_speed((epoch - 1) * len(train_loader) + iter + 1,
                        batch_time.avg,
...
def siamrpn_train(train_loader,
    ...
    for iter, input in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm(model.parameters(), 10)  # gradient clip，梯度裁剪
...
def siamiso_train(train_loader, model, optimizer, epoch, cur_lr, cfg,
    ...
    for iter, input in enumerate(train_loader):
        ...
        search = Variable(input[1]).cuda()
        label_cls = Variable(label_cls).cuda()
        weight = Variable(input[4]).cuda()

        
        out = model(template,search)
        loss = criterion(out,label_cls,weight)
        loss = torch.mean(loss)
...
# record loss
        ...
        if (iter + 1) % cfg.PRINT_FREQ == 0:
            ...
            tensor_writer.add_scalar('epoch_{0}_loss'.format(epoch),losses.avg,iter+1)
            tensor_writer.add_scalar('total_loss',losses.avg,epoch*len(train_loader)+iter)
            tensor_writer.add_scalar('batch_time',batch_time.avg,epoch*len(train_loader)+iter)
...
def unfix_more(model, optimizer, epoch, cfg, cur_lr, logger):
    # if model.module.features.unfix(epoch / cfg.SIAMFC.TRAIN.END_EPOCH): # 组成训练
    if True:
