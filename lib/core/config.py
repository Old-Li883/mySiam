...
config.SIAMFC.DATASET.GOT10K = edict(
)  # not utlized in paper but recommended, better performance and more stable
...
########config siamISO
# #-----————- config for siamfc ------------
config.SIAMISO = edict()
config.SIAMISO.TRAIN = edict()
config.SIAMISO.TEST = edict()
config.SIAMISO.TUNE = edict()
config.SIAMISO.DATASET = edict()
config.SIAMISO.DATASET.VID = edict()  # paper utlized but not recommended
config.SIAMISO.DATASET.GOT10K = edict()  # not utlized in paper but recommended, better performance and more stable


# augmentation
config.SIAMISO.DATASET.SHIFT = 4
config.SIAMISO.DATASET.SCALE = 0.05
config.SIAMISO.DATASET.COLOR = 1
config.SIAMISO.DATASET.FLIP = 0
config.SIAMISO.DATASET.BLUR = 0
config.SIAMISO.DATASET.ROTATION = 0

# vid
config.SIAMISO.DATASET.VID.PATH = '/mnt2/datasets/pysot_train/training_dataset/got10k/crop511'
config.SIAMISO.DATASET.VID.ANNOTATION = '/mnt2/datasets/pysot_train/training_dataset/got10k/train.json'

# got10k
config.SIAMISO.DATASET.GOT10K.PATH = '/mnt2/datasets/pysot_train/training_dataset/got10k/crop511'
config.SIAMISO.DATASET.GOT10K.ANNOTATION = '/mnt2/datasets/pysot_train/training_dataset/got10k/train1.json'

config.SIAMISO.TRAIN.MODEL = "SiameseISO"
config.SIAMISO.TRAIN.RESUME = False
config.SIAMISO.TRAIN.START_EPOCH = 0
config.SIAMISO.TRAIN.END_EPOCH = 50
config.SIAMISO.TRAIN.TEMPLATE_SIZE = 25
config.SIAMISO.TRAIN.SEARCH_SIZE = 65
config.SIAMISO.TRAIN.STRIDE = 1
config.SIAMISO.TRAIN.BATCH = 8
config.SIAMISO.TRAIN.PAIRS = 200000
config.SIAMISO.TRAIN.PRETRAIN = 'resnet23_inlayer.model'
config.SIAMISO.TRAIN.LR_POLICY = 'log'
config.SIAMISO.TRAIN.LR = 0.001
config.SIAMISO.TRAIN.LR_END = 0.0000001
config.SIAMISO.TRAIN.MOMENTUM = 0.9
config.SIAMISO.TRAIN.WEIGHT_DECAY = 0.0001
config.SIAMISO.TRAIN.WHICH_USE = 'GOT10K'  # VID or 'GOT10K'

# test
config.SIAMISO.TEST.MODEL = config.SIAMISO.TRAIN.MODEL
config.SIAMISO.TEST.DATA = 'VOT2015'
config.SIAMISO.TEST.START_EPOCH = 30
config.SIAMISO.TEST.END_EPOCH = 50

# tune
config.SIAMISO.TUNE.MODEL = config.SIAMISO.TRAIN.MODEL
config.SIAMISO.TUNE.DATA = 'VOT2015'
config.SIAMISO.TUNE.METHOD = 'GENE'  # 'GENE' or 'RAY'
...
def update_config(config_file):
    ...
    with open(config_file) as f:
        ...
        if model_name not in ['SIAMFC', 'SIAMRPN','SIAMISO']:
