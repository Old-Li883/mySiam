...
def load_dataset(dataset):
    ...
    if 'OTB' in dataset:
        ...
        for v in info.keys():
            # 每个视频建一个大字典
            path_name = info[v]['name'] # 文件夹名，待跟踪对象
            info[v]['image_files'] = [join(base_path, path_name, 'img', im_f) for im_f in info[v]['image_files']] # 所有文件名
            info[v]['gt'] = np.array(info[v]['gt_rect'])-[1, 1, 0, 0] # groundtruth名
            info[v]['name'] = v
...
def load_pretrain(model, pretrained_path):
    ...
    # 将预训练参数导入自己的模型中
    model.load_state_dict(pretrained_dict, strict=False) # False代表预训练模型和当前定义的模型不需要完全一致
    return model
...
def aug_apply(bbox, param, shape, inv=False, rd=False):
    ...
    if not inv:
        ...
        if 'scale' in param:
            ...
            # 放缩因子的确定，bbox(gt)放缩后不能超过原图，后面哪一项是可以放缩的最大倍数
