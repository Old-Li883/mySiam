...
class SiamFC(object):
    def __init__(self, cfg):
        ...
        self.info = cfg   # model and benchmark info，info包含网络结构信息，数据集信息，以及测试时是不是每个epoch都测试
    ...
    def init(self, im, target_pos, target_sz, model, hp=None):
        ...
        wc_z = target_sz[0] + 0.5 * sum(target_sz)
        hc_z = target_sz[1] + 0.5 * sum(target_sz)
        s_z = round(np.sqrt(wc_z * hc_z)) # 模板图缩放放后的（变成正方形）的边长

        # 将模板图缩放的缩放系数
        scale_z = self.info.SIAMISO.TRAIN.TEMPLATE_SIZE / s_z

        # 制作模板图像，填充均值，这里target_pos要转成cx、cy、w、h的形式
        z_crop = get_subwindow_tracking(im, target_pos, self.info.SIAMISO.TRAIN.TEMPLATE_SIZE, s_z, avg_chans)

        # 固定大小后模板图（127*127）相对于搜索区域（256*256）的padding（理解：就是127*127到256*256一边要填充多少，为后面多尺度做准备）
        d_search = (self.info.SIAMISO.TRAIN.SEARCH_SIZE - self.info.SIAMISO.TRAIN.TEMPLATE_SIZE) // 2
        pad = d_search / scale_z # 放缩后模板对应的padding
        s_x = s_z + 2 * pad # 加上padding后搜索图在原图的大小
...
class FCConfig(object):
    ...
    exemplar_size = 25 # 模板大小
    instance_size = 65 # 搜索区域大小
    score_size = 41 # 分数图大小
    total_stride = 1 # 下采样倍数
