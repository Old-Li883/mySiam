...
class BasicConv2d_1x1(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.last_relu:
            return F.relu(x, inplace=True) # F参数无法保存
        else:
            return x
...
class BasicConv2d_3x3(nn.Module):
    ...
    def __init__(self, inplanes, planes, last_relu=True):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False) # 1*1的卷积
        self.bn1 = nn.BatchNorm2d(planes) # 输入通道数
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = nn.Conv2d(planes, planes * self.expansion, kernel_size=1, bias=False) # 1 * 1卷积，并放大通道数
        self.bn3 = nn.BatchNorm2d(planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.last_relu = last_relu
...
class InceptionM(nn.Module):
    ...
    def forward(self, x):
        ...
        return center_crop(torch.cat(outputs, 1))   # cat按列拼接，in-layer crop，center_crop让torch在底层存储上连续
...
class Inception(nn.Module):
    ...
    def __init__(self, block, layers):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=0, bias=False) #  253*253*64
        ...
        # block为自定义的模块
        self.layer1 = self._make_layer(block, 64, 64, layers[0], pool=False)     # in=64, out=320（64*5）
        self.layer2 = self._make_layer(block, 320, 128, layers[1], pool=True, last_relu=False)   # in=320, out=640（128*5）

        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal(m.weight, mode='fan_out') # 正态化参数，OUT在反向传播时保持
            elif isinstance(m, nn.BatchNorm2d):
                nn.init.constant(m.weight, 1) # 使用值1填充网络
                nn.init.constant(m.bias, 0)
    ...
    def _make_layer(self, block, inchannels, planes, blocks, pool=True, last_relu=True):
        ...
        for i in range(0, blocks):
            ...
            if i == blocks - 1 and not last_relu: # blocks的最后一个block不要relu
